        ...
        <div className='px-31 max-w-360 mx-auto mt-18'>
            ...
            <div className='grid grid-cols-4 mt-12 gap-8'>
                ...
                {categories.map((item, index) => (
                    <div key={index} className='p-8 border border-[#D6DDEB] cursor-pointer'>
                        <Image src={item.icon} alt={item.label} width={48} height={48} />

                        <h1 className=' capitalize font-clash-display font-bold text-[24px] mt-8 text-natural100'>{item.label}</h1>

                        <p className='font-epilogue font-normal mt-3 text-[18px] text-[#7C8493]'>{item.des}</p>
